    ...
    <div className="w-full bg-slate-900/90 px-2 py-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {restaurentDetails?.map((restaurent) => (
          <div className="bg-gray-300 w-full sm:basis-96 lg:basis-72" key={restaurent.image_url}>
            <img src="/assets/trip_details.webp" alt="" className="" />
            <div className="flex flex-col py-3">
              <Link to={`https://www.google.com/maps/search/?api=1&query=${restaurent.name}`} target="_blank">
              ...
              <div className="flex flex-wrap gap-2 ml-2">
                {restaurent.amenities.map((aminity) => (
                  <p className="text-gray-300 bg-gray-900 w-fit px-2  capitalize" key={aminity}>
                    {aminity}
                  </p>
                ))}
              </div>
